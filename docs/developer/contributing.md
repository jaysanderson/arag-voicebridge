...
`docs/developer/settings.md` is not generated by a script, but it is transcribed from one source
the same way: `SETTINGS_FIELDS` in `src/services/settings.ts`. Adding, renaming or re-describing a
setting means updating both in the same change — the page says as much at the top precisely so a
reviewer can hold it to that.
