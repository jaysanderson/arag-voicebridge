/**
 * The reusable voice-answer prompt, applied inline on every `/ask`.
 *
 * Verified against a live KB: putting the grounding rules in the *system* slot makes the
 * generative model (a) refuse with the HANDOFF sentinel when the context doesn't cover the
 * question (it stopped answering "capital of France" from world knowledge), and (b) speak 2–3
 * short sentences with no markup. The `{context}`/`{question}` placeholders are substituted by
 * ARAG's prompt templater.
 */
...
/** The stored-search-configuration form of the same prompt (provisioning). */
export function voicePromptForSearchConfig(displayName: string, locale: string): string {
  const { system, user } = buildVoicePrompt(displayName, locale);
  return `${system}\n\n${user}`;
}
