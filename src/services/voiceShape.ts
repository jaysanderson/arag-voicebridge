 ...
 * Voice-shaping.
 *
 * ARAG returns written prose. TTS must speak something natural: ≤ 3 sentences, no URLs, no
 * markdown, no citation markers. This is a defence-in-depth backstop — the voice-answer prompt
 * already asks for spoken shape, but we never trust the model to be perfect on something the
 * prospect will literally hear.
 */
...
/**
 * Split into sentences and keep the first N. Simple and robust: split on sentence-final
 * punctuation followed by whitespace. Good enough for the ≤3-sentence answers ARAG produces
 * under the voice prompt.
 */
...
/**
 * Full voice-shaping pipeline. Order matters: strip structure before clamping so a stripped
 * URL doesn't leave a dangling "sentence".
 */
