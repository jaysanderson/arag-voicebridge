...
import { mintLiveKitToken, newRoomName } from "../src/livekit.ts";
import { createHmac } from "node:crypto";

function decodeJwt(token) {
  const [h, p, s] = token.split(".");
  const json = (seg) => JSON.parse(Buffer.from(seg, "base64url").toString("utf8"));
...
describe("mintLiveKitToken", () => {
  ...
  it("produces a verifiable HS256 JWT with the LiveKit video grant", () => {
    ...
    const expected = createHmac("sha256", base.apiSecret)
      .update(signingInput)
      .digest("base64url");
