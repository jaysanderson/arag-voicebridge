/**
 * Deterministic handoff detection (SPEC §8.4, §6.2.2 step 7, S6).
 *
 * This is a CONTRACT with the voice-answer prompt (docs/voice-answer-prompt.md), not a
 * fuzzy heuristic. The prompt is instructed to begin any unanswerable reply with a fixed
 * sentinel; the bridge keys `handoff` off that exact prefix.
 *
 * Belt-and-braces: an empty/ungrounded ARAG response (no answer text, no retrieval) is
 * ALSO treated as handoff, so a model that ignores the sentinel still degrades safely.
 */

/** Must stay identical to the sentinel in docs/voice-answer-prompt.md. */
...
export function decideHandoff(answerText: string, retrievalCount: number): HandoffDecision {
  ...
  if (trimmed.length === 0) {
    return { handoff: true, reason: "empty-answer" };
  }

  // 3. Nothing retrieved → answer (if any) is ungrounded; refuse rather than risk it.
  if (retrievalCount === 0) {
    return { handoff: true, reason: "no-retrieval" };
  }
...
/**
 * Strip the sentinel prefix from text (used only if we ever surface the model's own
 * handoff line; normally the bridge substitutes the prospect's configured handoff_msg).
 */
